...
export default function EquipScale() {
  ...
  return (
    <ul className="flex -gap-10 w-full justify-center items-center mt-28">
      {Lists.map((item) => (
        <li
          onClick={() => setActive(item.id)}
          key={item.id}
          className={`${active === item.id ? "scale-125 -mr-6 ml-5 " : " custom-effect-scale"}  duration-200 ease-in flex-shrink-0 -mr-12 clip-path-parallelogram w-[222.67px] `}
        >
