...
export default function Slider() {
  ...
  return (
    <>
      {dataSlider.map((item) => (
        <li
          className={` ${active === item.id ? "opacity-1" : "opacity-0"} z-20 transform ease-in duration-300 list-none absolute flex right-[174px] bottom-[320px] `}
          key={item.id}
        >
      ...
      <MdKeyboardArrowLeft
        size={86}
        color="#8E8E97"
        className=" absolute left-[568px] bottom-96 cursor-pointer"
        onClick={() => handleActiveSlide("left")}
      />
      <MdKeyboardArrowRight
        size={86}
        color="#8E8E97"
        className=" absolute right-[68px] bottom-96 cursor-pointer"
