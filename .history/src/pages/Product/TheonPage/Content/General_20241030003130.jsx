...
export default function General() {
  ...
  return (
    <div className="h-[1530] px-10">
      <h1 className="text-[1.5rem] tracking-[1rem] uppercase  text-[#8E8E97]">
        vinfast - theon s
      </h1>
      <ul className="w-[514px] border-2 ">
        {Lists.map((item, index) => (
          <li key={index} className="flex gap-32 font-bold text-[1.3rem] mt-5">
            <h1>{item.title1}</h1>
            <h1 className="order-last">{item.title2}</h1>
          </li>
        ))}
        <p className="text-[0.938rem] font-light">
          Đã bao gồm VAT, Không giới hạn quãng đường di chuyển
        </p>
        <Button className="shadow-customButton w-[440px]">
