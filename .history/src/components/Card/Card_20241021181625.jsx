    ...
    <>
      {props.DataCard && // Kiểm tra xem DataCard có tồn tại không
        (props.List !== undefined
          ...
          .map((item, index) => (
            <div
              className={`${(index == 3 && props.DataCard.filter((item) => item.list === props.List).length <= 6) || index == 1 || index == 6 ? "snap-start" : ""} relative w-[440px] h-[452px] bg-gradient-to-b from-[#E9E9E9] to-transparent mr-5`}
              key={index}
            >
